...
export function IntegrityTrustModal({ isOpen, onClose }: IntegrityTrustModalProps) {
  ...
  return (
    ...
    >
      <div
        ref={modalRef}
        className="relative w-full max-w-2xl max-h-[88vh] bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-2xl overflow-hidden flex flex-col my-auto transition-colors"
      >
        {/* Modal Sticky Header */}
        <div className="sticky top-0 z-20 px-5 sm:px-6 py-4 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md border-b border-slate-100 dark:border-slate-800 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl bg-blue-100 dark:bg-blue-950/80 text-blue-600 dark:text-blue-400 border border-blue-200 dark:border-blue-800/60 flex items-center justify-center shrink-0">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Transparansi perlindungan privasi pada saluran pengaduan integritas
              </p>
            </div>
          </div>
          ...
          <button
            type="button"
            onClick={onClose}
            className="p-2 rounded-xl text-slate-400 hover:text-slate-700 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors cursor-pointer min-h-[44px] min-w-[44px] flex items-center justify-center"
            title="Tutup dialog"
        ...
        <div className="p-6 sm:p-8 overflow-y-auto space-y-7 text-xs sm:text-sm text-slate-700 dark:text-slate-300">
          {/* ── 1. Core Trust Statement ──────────────────────────────────── */}
          <div className="p-4 rounded-2xl bg-blue-50/90 dark:bg-blue-950/40 border border-blue-200/90 dark:border-blue-800/60 text-blue-950 dark:text-blue-200 flex items-start gap-3.5">
            ...
            <div className="grid grid-cols-1 gap-3">
              ...
              <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-950 border border-slate-200/80 dark:border-slate-800 space-y-1.5">
                ...
                <div className="pl-7 flex flex-wrap gap-2 pt-1">
                  {['Nama Pelapor', 'Email', 'NIK / Employee ID', 'Akun WACT'].map((item) => (
                    <span
                      key={item}
                      className="px-2.5 py-1 rounded-lg bg-rose-50 dark:bg-rose-950/60 border border-rose-200 dark:border-rose-800/60 text-rose-700 dark:text-rose-400 text-[11px] font-semibold flex items-center gap-1"
                    >
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <h3 className="text-xs font-extrabold uppercase tracking-wider text-slate-900 dark:text-white flex items-center gap-2">
                ...
                <span>Contoh tampilan yang diterima tim investigator</span>
              </h3>
            ...
            {/* Live Mockup Box */}
            <div className="p-4 sm:p-5 rounded-2xl bg-slate-100/90 dark:bg-slate-950 border-2 border-slate-200 dark:border-slate-800 space-y-4">
              {/* Mockup Header */}
              <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-200 dark:border-slate-800 pb-3">
                ...
                <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-rose-100 dark:bg-rose-950/70 border border-rose-300 dark:border-rose-800 text-rose-800 dark:text-rose-300 text-[10.5px] font-extrabold">
                  <Lock className="w-3 h-3 text-rose-600 dark:text-rose-400" />
                  <span>Identitas Pelapor Tidak Tersedia</span>
                </div>
              </div>
              ...
              <div className="p-3 rounded-xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800/80 space-y-2">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 text-[11px]">
                  <div className="p-1.5 rounded-lg bg-slate-50 dark:bg-slate-950 border border-slate-100 dark:border-slate-800/60">
                    <span className="text-slate-400 block text-[9.5px]">Nama Pelapor:</span>
                    <span className="font-bold text-rose-600 dark:text-rose-400">ANONIM (Tidak Ada)</span>
                  </div>
                  <div className="p-1.5 rounded-lg bg-slate-50 dark:bg-slate-950 border border-slate-100 dark:border-slate-800/60">
                  ...
                  <div className="p-1.5 rounded-lg bg-slate-50 dark:bg-slate-950 border border-slate-100 dark:border-slate-800/60">
                    <span className="text-slate-400 block text-[9.5px]">NIK / ID Karyawan:</span>
                    <span className="font-bold text-slate-500">Tidak tersedia</span>
                  </div>
                  <div className="p-1.5 rounded-lg bg-slate-50 dark:bg-slate-950 border border-slate-100 dark:border-slate-800/60">
                    <span className="text-slate-400 block text-[9.5px]">Akun WACT:</span>
                  ...
                  <div className="p-1.5 rounded-lg bg-slate-50 dark:bg-slate-950 border border-slate-100 dark:border-slate-800/60">
                    <span className="text-slate-400 block text-[9.5px]">User Agent / Device:</span>
                    <span className="font-bold text-slate-500">Tidak dicatat</span>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-1 text-xs">
                <div className="flex items-center justify-between text-[11px] text-slate-500">
                  <span>Kategori: <strong>Konsumsi Barang Tanpa Izin</strong></span>
                  <span>Area: <strong>Picking / PTL</strong></span>
                </div>
                <div className="p-2.5 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800/60 text-[11px] text-slate-700 dark:text-slate-300 leading-relaxed italic">
          ...
          <div className="space-y-2.5 p-4 rounded-2xl bg-slate-50 dark:bg-slate-950 border border-slate-200/80 dark:border-slate-800">
            <h3 className="text-xs font-extrabold uppercase tracking-wider text-slate-900 dark:text-white flex items-center gap-2">
            ...
            <ul className="space-y-1.5 text-xs text-slate-600 dark:text-slate-400 list-disc pl-4 leading-relaxed">
              <li>Pembersihan (*sanitization*) metadata dilakukan secara otomatis di server WACT.</li>
              <li>Seluruh data <strong>EXIF, koordinat GPS, model kamera, dan waktu perangkat</strong> dihapus.</li>
              <li>Nama file asli (*original filename*) tidak disimpan dan diganti dengan nama acak terenkripsi.</li>
            </ul>
          </div>

          {/* ── 5. Reporter Self-Identification Warning ──────────────────── */}
          <div className="p-4 rounded-2xl bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800/60 text-amber-900 dark:text-amber-200 flex items-start gap-3">
        ...
        {/* Modal Footer */}
        <div className="sticky bottom-0 z-20 px-6 py-4 bg-slate-50 dark:bg-slate-900/90 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between gap-3">
          <span className="text-[11px] text-slate-500">
            WACT Whistleblower Protection Standard
          </span>
          <button
            type="button"
            onClick={onClose}
            className="py-2 px-5 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-bold text-xs shadow-md shadow-blue-500/20 transition-all cursor-pointer min-h-[44px]"
          >
            Mengerti & Tutup
          </button>
        </div>
